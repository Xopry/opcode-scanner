...
class PacketInfo {
	...
	parse(strictLength) {
		...
		if (msg)
			try {
				...
				this.parsed = protocol.parse(
					this.version,
					msg,
					this.data,
					name
				);
				this.parsedLength = protocol.write(
					this.version,
					msg,
					'*',
					this.parsed,
		...
		if (
			this.parsed &&
			(!strictLength || this.parsedLength === this.data.length)
		)
			return true;
	...
	first(name) {
		...
		for (let packet of this.history)
			if (packet.name() === name) return packet;
	}
...
module.exports = function OpcodeScanner(dispatch) {
	...
	dispatch.hook(
		...
		(code, data, fromServer) => {
			if (!version) {
				...
				let files = fs
					.readdirSync(path.join(__dirname, 'patterns'))
	...
	(async () => {
		...
		while (connected) {
			...
			for (let packet of history)
				if (!packet.parsed) {
					if (
						Date.now() - sleepTime > PASSIVE_SCAN_TIMEOUT &&
						connected
					) {
			...
			if (name.startsWith(prefix)) {
				...
				if (patterns[name](info)) {
					...
					if (info.parsedLength === info.data.length) {
						...
						!(loggedMatch[name] || (loggedMatch[name] = []))[
							info.code
						]
					) {
	...
	function writeMap() {
		...
		fs.writeFileSync(
			path.join(mapsDir, 'protocol.' + version + '.map'),
			out.join('\n')
		);
	}
